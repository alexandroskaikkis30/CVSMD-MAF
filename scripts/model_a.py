...
# The conditioning variable is the positively directed pulling force recorded for one of the terminal atom
COND_COL = "pulling_force_x_terminal_atom_positive"

# Directory paths
npy_dir = "/path/to/trajectories"
csv_dir = "/path/to/logs"
base_save_path = "/path/to/project"
save_path = os.path.join(base_save_path, "model_a")
...
# Load trajectory data, originally saved as 3D array of shape (num_atoms, timesteps, 3)
   # last dimension corresponds to Cartesian coordinates (x, y, z)
    ...
    if "iter" not in df.columns or COND_COL not in df.columns:
        continue
    ...
    cond_vals = df[COND_COL].values[:T].astype(np.float32)
    if len(cond_vals) != T:
        continue

    cond = cond_vals.reshape(T, 1)
...
if not X_val_list or not cond_val_list:
    ...
    cond_val_list = [np.empty((0, 1), dtype=np.float32)]
...
# with 128 hidden features and 1 conditioning input. The base distribution is multivariate standard normal
